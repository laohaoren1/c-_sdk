...
`send()` 和 `stop()` 共享 `socket_mutex_`，通过互斥保证不会向已关闭的 fd 写数据：

```cpp
void CANMessageDispatcher::send(const CanMessage& msg) {
  std::lock_guard<std::mutex> lock(socket_mutex_);

  // 持锁检查 fd：与 stop() 互斥，stop() 关闭 socket 后 fd = -1
  if (socket_fd_ < 0) {
...
`CANMessageDispatcher` 中的锁各自保护独立的资源，不存在同时持有多个锁的场景：

```
┌──────────────────────────────────────────────────────────────────┐
│  subscribers_mutex_       保护 subscribers_ 列表的增删和拷贝      │
│  使用者：subscribe() / unsubscribe() / recv_loop() 拷贝快照     │
├──────────────────────────────────────────────────────────────────┤
│  socket_mutex_            保护 socket_fd_ 的读写和关闭           │
│  使用者：send() / stop() 关闭 socket                            │
├──────────────────────────────────────────────────────────────────┤
...
std::atomic<bool> running_{false};          // CANMessageDispatcher：控制 recv_loop 退出
std::atomic<bool> streaming_running_{false}; // Manager：控制 streaming_loop 退出

// 内存序选择
running_.store(true, std::memory_order_release);   // 写操作（发布新状态）
running_.load(std::memory_order_acquire);          // 读操作（获取最新状态）

// SubscriberState 中的活跃标志
std::atomic<bool> active{true};
active.store(false, std::memory_order_release);    // deactivate：发布
active.load(std::memory_order_acquire);            // recv_loop 中检查
```

**注意**：`Lifecycle` 的状态（`LifecycleState`）不使用原子变量，而是通过 `std::mutex` 保护，因为状态变更需要同时通知订阅者，属于复合操作。
...
添加 L10 后，更新 `linkerhand.hpp` 统一入口：

```cpp
// include/linkerhand/linkerhand.hpp（添加 L10 后的样子）
#pragma once

#include "linkerhand/hand/l6/l6.hpp"
#include "linkerhand/hand/l10/l10.hpp"  // 新增

namespace linkerhand {

// 便捷别名
using L6 = hand::l6::L6;
using L10 = hand::l10::L10;  // 新增
